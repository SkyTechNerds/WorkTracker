...
const api = {
  ...
  setAbsence: (fromMs: number, toMs: number, type: 'krank' | 'urlaub' | 'fza' | null) => ipcRenderer.invoke('set-absence', fromMs, toMs, type),
  ...
  exportReport: (from: string, to: string) => ipcRenderer.invoke('export-report', from, to),
  checkUpdate: () => ipcRenderer.invoke('check-update'),
  openExternal: (url: string) => ipcRenderer.invoke('open-external', url),
  appVersion: () => ipcRenderer.invoke('app-version'),
  popupResult: (kind: string, value: string, payload?: unknown) => ipcRenderer.invoke('popup-result', kind, value, payload),
